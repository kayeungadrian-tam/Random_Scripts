...
class Course():
    def __init__(self):
        theta = np.linspace(-np.pi/2, 0, 40000)
        r = np.sqrt(radius**2)
        ...
        xs2 = np.array([300]*1000)
        ys2 = np.linspace(260,400, 1000)
        upper = np.hstack((xs2.reshape(-1,1),ys2.reshape(-1,1)))

        final = np.concatenate((lower,result,upper))
...
def sampling(total_steps, out_path, apply_nn=False):
    ...
    def update_state_vector(idx,s):
        ...
        if s.x < 50:
            state_vector[idx][12] = 0 # 曲率（コース）
        elif s.y > 260:
            state_vector[idx][12] = 0 # 曲率（コース）
        else:
            state_vector[idx][12] = 0.004 # 曲率（コース）
...
if __name__=='__main__':
    ...
    # sampling(4000,out_path=out_path)

    if Training:
        training(epoch=2000)
        
    sampling(4000, out_path=out_path, apply_nn=True)
