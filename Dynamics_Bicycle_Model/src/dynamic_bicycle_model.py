...
def plot_trajectory():
    ...
    ax.grid()
    fig.tight_layout()
    fig.savefig(out_path+'/01_Trajectory')
...
def write_csv(total_steps):
    ...
    excel = pandas.read_excel(f'{out_path}/00_log-{datetime.date.today()}.xlsx', index_col=0)
    excel.to_csv(f'{out_path}/00_log-{datetime.date.today()}.csv')
...
if __name__=='__main__':
    ...
    for i in tqdm(range(total_steps)):
        ...
        # print(f'{nn_next:.3f}\t{feedback_value:.3f}')
